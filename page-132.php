<?php
/**
 * Главный шаблон для страницы рейтинга застройщиков без вкладок
 */
...
$ids = wp_list_pluck($page_fields['tabs'][0]['list'], 'ID');
...
<?php
// === Добавим баннер внизу страницы ===
$bottom_banner_data = null;

if (!empty($page_fields['banners']) && is_array($page_fields['banners'])) {
    $bottom_banner_data = $page_fields['banners'][0]; // можно сделать фильтрацию по позиции
}

if ($bottom_banner_data && !empty($bottom_banner_data['image'])) {
    $args = [
        'bottom_banner_pk'  => $bottom_banner_data['image'],
        'bottom_banner_mob' => $bottom_banner_data['image'], // можно дублировать или сделать отдельное поле для мобилки
